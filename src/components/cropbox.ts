class CropBox {
    ...
    registerCropboxScaleEvents() {
        ...
        this.boders.forEach((border, index) => {
            border.addEventListener("mousedown", (e) => {
                ...
                this.mouseInfo.mouseY = e.clientY;
                this.mouseInfo.type = "scale";
                this.mouseInfo.index = index === 0 ? 1:  index === 1 ? 4 : index === 2 ? 6 : 3;
                this.originalPosition.x = this.position.x;
                this.originalPosition.y = this.position.y;
                this.originalPosition.width = this.position.width;
                this.originalPosition.height = this.position.height;
    ...
    initPointer() {
        ...
        this.pointerContainer.setAttribute("class", "video-cropper-crop-box-pointer-container");    
    ...
    initGrid() {
        ...
        this.gridContainer.setAttribute("class", "video-cropper-crop-box-grid-container");    
        this.grids = Array(9).fill(null);
        this.grids.forEach((_, index) => {
            const grid = document.createElement("div")
            grid.setAttribute("class", `video-cropper-crop-box-grid-${index} video-cropper-crop-box-grid`);
    ...
    initBorder() {
        ...
        this.broderContainer.setAttribute("class", "video-cropper-crop-box-border-container");
        ...
        this.boders = this.boders.map((_, index) => {
            ...
            border.setAttribute("class", `video-cropper-crop-box-border-${index} video-cropper-crop-box-border`);
    ...
    cropboxScale(e: MouseEvent) {
        switch(this.mouseInfo.index) {
            case 0: {
                this.position.x = this.originalPosition.x + (e.clientX - this.mouseInfo.mouseX);
                this.position.y = this.originalPosition.y + (e.clientY - this.mouseInfo.mouseY);
                this.position.width = this.originalPosition.width - (e.clientX - this.mouseInfo.mouseX);
                this.position.height = this.originalPosition.height - (e.clientY - this.mouseInfo.mouseY);
                break;
            }
            case 1: {
                this.position.y = this.originalPosition.y + (e.clientY - this.mouseInfo.mouseY);
